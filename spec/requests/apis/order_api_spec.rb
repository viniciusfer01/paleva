...
describe 'Order API' do 
  context "Get /api/orders" do
    ...
    context "Post /api/orders/:id/prepping?code=store_code" do
      it 'succesfully' do 
        user = User.create!(cpf: '66101052001', name: 'Zezin', last_name: 'do Teclados', 
                            email: 'zezin@teclados.com', password: 'passwordpass')
        ...
        post "/api/orders/#{order.code}/prepping?code=#{store.code}"

        expect(response.status).to eq 200  
        expect(response.content_type).to include 'application/json'  
        expect(response.body).to include(order.code)   
        expect(response.body).to include('prepping')   
      end
    end
    context "Post /api/orders/:id/ready?code=store_code" do
      it 'succesfully' do 
        user = User.create!(cpf: '66101052001', name: 'Zezin', last_name: 'do Teclados', 
                            email: 'zezin@teclados.com', password: 'passwordpass')
        ...
        post "/api/orders/#{order.code}/ready?code=#{store.code}"
